...
// LIBRARY DETAIL
// =========================
export default function LibraryDetail({ darkMode, toggleDarkMode, user, logout }) {
  const { id } = useParams()
  ...
  // =========================
  return (
    <div className={`min-h-screen p-8 ${darkMode ? "bg-[#0B0E1D] text-white" : "bg-[#EAF1FF] text-black"}`}>
      <Header
        introProps={{
          user: user,
      ...
      <div className="flex flex-col md:flex-row gap-8 mt-8">
        ...
        <div className="w-full md:w-1/2 flex flex-col gap-4">
          ...
          <p className="text-gray-400 dark:text-gray-300">{library.description}</p>
          <p className="text-sm text-gray-400 dark:text-gray-300">
            Created on {new Date(library.created_at).toLocaleDateString()} by {library.user_name || "Unknown"}
          </p>
        </div>
      </div>
